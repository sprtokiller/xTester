...
const rules2: FormRules = {
  testerUUIDs: {
    ...
    validator() {
      msgOpacity.value =
        formValue2.value?.testerUUIDs.length +
          formValue2.value?.groupUUIDs.length +
...
function renderLabel(option: SelectOption) {
  return [
    ...
    h('span', { style: { color: 'var(--gray-3)', marginLeft: '0.5rem' } }, [
      ...
      h(
        NIcon,
        { size: 'medium', style: { verticalAlign: '-0.15em' } },
        { default: () => h(PersonFilled) }
      )
    ])
  ]
}
...
async function preview() {
  ...
  courseName.value = courseOptions.value.find(
    (course) => course.value === formValue1.value.courseUUID
  )?.label as string
  formatedTime.value =
    dayjs(formValue1.value.startAt).format('YYYY-MM-DD HH:mm:ss') +
    ' - ' +
    dayjs(formValue1.value.startAtEndAt[1]).format('YYYY-MM-DD HH:mm:ss')
  try {
    uploading.value = true
    previewTesters.value = await API.getTesterPreview(
      formValue2.value.testerUUIDs,
      formValue2.value.groupUUIDs
    )
  } catch (err) {
...
onMounted(async () => {
  try {
    loading.value = true
    const data = await Promise.all([API.getCourseList(), API.getTesterList(), API.getGroupList()])
          ...
          <n-card style="width: 100%" v-if="step === 1" title="Step 1: Settings">
            <n-form ref="formRef1" :model="formValue1" :rules="rules1">
              <n-space vertical>
                <n-form-item path="courseUUID" label="Course">
                  <n-select
                    v-model:value="formValue1.courseUUID"
                    filterable
                ...
                <n-form-item path="endType" label="When should the test end?">
                  <div class="d-flex flex-row flex-wrap align-items-center">
                    <n-radio-group
                      v-model:value="formValue1.endType"
                      name="RBG1"
                ...
                <n-form-item
                  v-if="formValue1.endType == 'MANUAL'"
                  path="startAt"
                  label="Test will start at:"
                >
            ...
            <template #action>
              <div class="d-flex justify-content-end">
                <n-button @click.stop="cancel" style="margin-right: 0.5rem" ghost :disabled="error"
                  >Cancel</n-button
                >
                <n-button @click.stop="goToStep(2, 1)" type="primary" :disabled="error"
                  >Continue</n-button
                >
              </div>
            </template>
            ...
            <n-form ref="formRef2" :model="formValue2" :rules="rules2">
              <n-space vertical>
                <span class="hint-label"
                  >Select testers or entire tester groups. Overlaps will be merged.</span
                >
                <n-form-item path="testerUUIDs" label="Testers">
                  <n-select
                    v-model:value="formValue2.testerUUIDs"
                    filterable
                    multiple
                ...
                <n-form-item path="groupUUIDs" label="Groups">
                  <n-select
                    v-model:value="formValue2.groupUUIDs"
                    filterable
                    multiple
                    :options="groupOptions"
                    :render-label="renderLabel"
                    placeholder="...or groups"
                  />
                </n-form-item>
                <span class="hint-label"
                  >You can also add anonymous testers, but you will have to send them the link to
                ...
                <n-form-item path="anonymousTesterCount" label="Anonymous testers">
                  <n-input-number
                    :min="0"
                    :max="1000"
                    v-model:value="formValue2.anonymousTesterCount"
                    style="width: 100%"
                  />
                </n-form-item>
                <span class="error-label fade-opacity" :style="{ opacity: msgOpacity }"
                  >Please add some testers to your test.</span
                >
              </n-space>
            </n-form>
              ...
              <div class="d-flex justify-content-between">
                ...
                <div>
                  <n-button @click="cancel" style="margin-right: 0.5rem" ghost :disabled="error"
                    >Cancel</n-button
                  >
                  <n-button @click="goToStep(3, 2)" type="primary" :disabled="error"
                    >Preview</n-button
                  >
                </div>
              </div>
